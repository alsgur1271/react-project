...
import { createSession, fetchStudentList, fetchMyStudents } from '../../services/api';
    ...
    const loadStudents = async () => {
      try {
        ...
        try {
          ...
          console.log('할당된 학생 목록:', studentList);
        } catch (err) {
          console.error('할당된 학생 목록 로드 오류:', err);
          // 할당된 학생 가져오기 실패 시 모든 학생 목록 가져오기
          studentList = await fetchStudentList();
          console.log('모든 학생 목록:', studentList);
        }
