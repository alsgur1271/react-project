...
router.get('/users', async (req, res) => {
  try {
    const [rows] = await db.query(
      'SELECT id, username, email, role, created_at FROM users WHERE verified != 2 ORDER BY created_at DESC'
    );
...
router.get('/teachers', async (req, res) => {
  try {
    const [rows] = await db.query(
      'SELECT id, username, email, created_at FROM users WHERE role = "teacher" AND verified = 1 ORDER BY username'
    );
...
router.get('/students', async (req, res) => {
  try {
    const [rows] = await db.query(
      'SELECT id, username, email, created_at FROM users WHERE role = "student" AND verified = 1 ORDER BY username'
    );
...
router.post('/teachers/:teacherId/students', async (req, res) => {
  try {
    ...
    const [teachers] = await db.query(
      'SELECT id FROM users WHERE id = ? AND role = "teacher" AND verified = 1',
      [teacherId]
    );
